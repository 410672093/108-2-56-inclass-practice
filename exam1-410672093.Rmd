...
```{r ans1}
as.factor(receipts300)
type.convert(receipts300)
levels(as.factor(receipts300))
receipts300 <- c("૧"=1,"૨"=2,"૩"=3,"૪"=4,"૫"=5,"૬"=6,"૭"=7,"૮"=8,"૯"=9)
...
```{r ans21}
str(poem_songOrder)
poem_song[[4]] <- poem_song
poem_song[[1]] <- poem_song
poem_song[[2]] <- poem_song
poem_song[[3]] <- poem_song
poem_song[[6]] <- poem_song
poem_song[[7]] <- poem_song
poem_song[[8]] <- poem_song
poem_song[[10]] <- poem_song
poem_song[[11]] <- poem_song
poem_song[[12]] <- poem_song
poem_song[[4]] <- poem_song
poem_song[[4]] <- poem_song
 poem_song
```
...
poem_song[[22]]$paragraphs <- "斗柄仙城曉，龍鱗下隰通。 三竿眺羲馭，十里過商中。 水藉天容碧，塵依驛轍紅。 喜聞何力語，應爲樂秋豐。"
...
as.POSIXct(lubridate::mdy_hm(post$comments$DateTime,tz="Asia/Taipei"))
post <- mdy_hms(post$comments$DateTime,tz="Asia/Taipei")
...
#str(post)
with_tz(mdy_hms("4月25日 15:50",tzone="Europe/Poland"))
post$comments$gender[[26]] <- "男"
post$comments$DateTime[[26]] <- "4月25日 15:50"
...
post$category_dateTime[c(-2)] -> post$category_dateTime
post[[5]][["dateTime"]] -> post[5]
dateTime=mdy_hm("4月20日 14:44")
post[[6]]
 post
```
