...
```
```{r}
example$name[after98 & !pick_above170]
```
```{r}
install.packages("stringr","glue")
```
```{r}
library(stringr)
library(glue)
...
```{r}
str_sub(c("410672093","我很喜歡打羽球喔"),5,8)
#可以選元素，用在月份之類的不知道可不可以
#是始和終喔，幾號到幾號結束
```
```{r}
#民國年轉西元年
...
```{r}
str_extract_all(c("民國019年12月05日",    "民國108年12月5日","民國98年10月5日"),"(?<=民國)[0-9]{2,3}(?=年)"
)
#給他格式 用小括號，extract是叫出vector，extractall是要出list
str_extract(
  c("民國108年12月5日","民國98年10月5日"),
  "(?<=民國)[0-9]{2,3}(?=年)")
#(?<=民國)[0-9]{2,3}(?=年)
#開頭是民國，載來兩個數字是介於0-9，最後結尾是年
```
```{r}
install.packages("htmlwidgets")
```
```{r}
str_extract_all(c("民國108年12月5日","民國98年10月5日"),"(?<=民國)[0-9]{2,3}(?=年)")
#我沒辦法用VIEW，老師說用EXTRACT也可以
```
```{r}
...
str_extract_all(studentIds, "107|108") # 可不寫()
...
```{r}
Ids <- c("a410873029","b410773005","c410772035","d410562123")
str_extract(Ids,"[ac]")
...
str_extract_all(c(claimA,claimB,claimC),pattern = myRegexSlotMachine)
#從三個CLAIM中擷取出有pattern= "[0-9](櫻桃|777|紅心)[A-Z]"的文字內容~
...
```{r}
# 顯示所有符合規則的
str_view_all(headline, "謝陳官") # 固定字組"謝陳官
str_count(headline,"謝陳官")

str_extract_all(headline, "[謝陳官]") # 有1個strip: 出象有3個可能字元 謝、陳、官
str_count(headline, "[謝陳官]")

str_extract_all(headline, "謝無雙|官求敗") # 有1個strip，出象有2個字組 
str_count(headline, "謝無雙|官求敗")

str_extract_all(headline, "陳[^官]") # 固定字元＋有1個strip： 出象為排除官的字元
...
#paste0(c(strip1,strip2,strip3,strip4))
#為啥出現的是char分開的
...
str_extract_all(addresses,"[0-9]+") # 幾樓也會選到
#[0-9]+的意思是[0-9]，+，加號是可以有好多個但不知有幾個
```
```{r}
phrase <- c("唧唧复唧唧")
target_regex <- "唧唧"
pattern <- glue::glue("^{target_regex}")
#會在環境中把target_regex=唧唧 抓進來當開頭出現   ^唧唧
pattern <- glue::glue("{target_regex}$")
#兩個一在前一在後，前後是不一樣
pattern
...
year <- str_extract(c("民國108年12月5日","民國98年10月5日"),"(?<=民國)[0-9]{2,3}(?=年)")
#注意喔，extract_all是list!不要list
#想抓的是前=民國，後接著年的一串數字
#接著設定pattern
target_regex <- "[0-9]+"
A_regex <- "民國"
...
#小括號裡面裝鄰居   ?<={}，中間我要抓target_regex裡面的東西所以要用大括號，最後再給後面鄰居的形式，最後不要旁邊兩個鄰居顯示出來
regex_pattern
westernyear <- as.integer(year)+1911

#因為str是對char在運作的，所以要轉char
#replace的要件有變數，pattern，要替換的char，有""的
str_replace(twDate,regex_pattern,as.character(westernyear)) -> twDate_reformat#回存新的名字
twDate_reformat

date_final <- lubridate::ymd(twDate_reformat)
date_final
```
```{r}
